        ...
        for pair in POSE_PAIRS:
            ...
            if p1 and p2 and p2[0] != p1[0]:
                ...
                try :
                    ...
                    while(color_diff(list(outframe[p3[0], p3[1]]), inital_color) <= 20.0 or color_diff(list(outframe[int(p1[0] + dist * math.sin(theta)), int(p1[1] + dist * math.cos(theta))]), inital_color) <= 20.0):
                        p3[0] = int(p1[0] + dist * math.sin(theta))
                        p3[1] = int(p1[1] + dist * math.cos(theta))
                        dist += 1
                    dist = 0
                    inital_color = list(outframe[p4[0], p4[1]])
                    while(color_diff(list(outframe[p4[0], p4[1]]), inital_color) <= 20.0 or color_diff(list(outframe[int(p1[0] - dist * math.sin(theta)), int(p1[1] - dist * math.cos(theta))]), inital_color) <= 20.0):
